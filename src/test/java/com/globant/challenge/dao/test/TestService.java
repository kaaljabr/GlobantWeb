package com.globant.challenge.dao.test;
...
public class TestService {
    ...
    public void testFetchUsers(){      
        //make sure the dump is working fine and there are 11 records available in the DB
    	try {			
    ...
    public void testUsersByProfession(){      
        //make sure when searching for users who have profession accountant we have 3
    	try {			
    ...
    public void testUsersPaginationRecord(){      
        //make sure when returning paginated users the first record in the second page is walter
    	try {			
    ...
    public void testReadFilesIOException() throws IOException{      
    	...
    	try {			
			List<String> filesNames = fileService.readFiles("e:\\WrongName");
    ...
    public void testReadFiles(){      
        //make sure there is one file in the specified directory
    	try {			
			List<String> filesNames = fileService.readFiles("e:\\dev\\directory");
			Assert.assertEquals(filesNames.size(), 1);	
		} catch (ServiceException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
    ...
    public void testReadFileName(){      
    	...
    	try {			
			List<String> filesNames = fileService.readFiles("e:\\dev\\directory");
			Assert.assertEquals(filesNames.get(0), "test.txt");	
		} catch (ServiceException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
